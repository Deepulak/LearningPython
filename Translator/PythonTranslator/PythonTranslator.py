...
def CreateWidgets():
    languagesListLabel = Label(root, text="SELECT A LANGUAGE FROM THE LIST", bg="royalblue3")
    ...
    inputTextLabel = Label(root, text="TEXT TO DETECT : ", bg="royalblue3")
    ...
    findButton = Button(root, text="DETECT LANGUAGE", command=detectLanguage)
    findButton.grid(row=1, column=1, padx=5, pady=5, columnspan=2)

    detectedLanguageLabel = Label(root, text="DETECTED LANGUAGE : ", bg="royalblue3")
    ...
    i_tranlateTextLabel = Label(root, text="TEXT TO TRANSLATE : ", bg="royalblue3")
    i_tranlateTextLabel.grid(row=3, column=0, padx=10, pady=5)
    root.i_tranlateTextEntry = Entry(root, width=20, bg='snow3', textvariable=textToTranlate)
    root.i_tranlateTextEntry.grid(row=3, column=1, padx=10, pady=5)

    languageLabel = Label(root, text="SELECTED LANGUAGE : ", bg="royalblue3")
    languageLabel.grid(row=4, column=0, padx=10, pady=5)
    root.languageEntry = Label(root, width=20, bg='snow3')
    root.languageEntry.grid(row=4, column=1, padx=10, pady=5)

    translateButton = Button(root, text="TRANSLATE TEXT", command=translateText)
    translateButton.grid(row=5, column=1, padx=5, pady=5, columnspan=2)

    translatedTextLabel = Label(root, text="TRANSLATED TEXT", bg="royalblue3")
    translatedTextLabel.grid(row=6, column=0, padx=10, pady=5)
    root.translatedText = Label(root, width=20, bg='snow3')
    root.translatedText.grid(row=6, column=1, padx=10, pady=5)
...
# Defining detectLanguage() function to detect the language of the entered text
def detectLanguage():
    # Storing the input text to detect language
    inpuTextToDetect = textToDetect.get()
    # Detecting the language  of the entered text using the detect() method which returns the detected language in the form of shorthand notation
    detectedLanguage = tranlator.detect(inpuTextToDetect)
    detectedLanguage = detectedLanguage.lang
    # Using the returned shorthand notation fetching the language from the languagesList and displaying it tkinter window
...
def translateText():
    ...
    # Splitting the language on '-' and fetching the first item from the resulting list which corresponds to the SHORTHAND NOTATION OF THE LANGUAGE and storing it
    shorthand_notation = language.split('-')[0]
    # Translating the entered text to the selected language using the translate() which takes input text and shorthand notation of the language as arguments. 
...
# Fetching the languages supported by googletrans using LANGUAGES which returns dictionary of supported languages and storing it the languagesList variable
